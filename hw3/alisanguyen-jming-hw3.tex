...
\item Looking at the Harvard football team in the 2011-2012 season,
	\begin{itemize}
	\item Using the ML approach, we basically take the naive probability looking at the number of wins over the total number of games, so the probability they would win is $\frac{9}{10}$.
	...
	\[\theta_{MAP} = \frac{n_w+\alpha-1}{n+\alpha+\beta-2}=\frac{5+9-1}{10+5+3-2}=\frac{13}{16}\]. Beta(5,3) is good because it accounts for complications about uniformity and symmetry.
