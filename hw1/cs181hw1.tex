...
\item In this example, a tree that could be formed would split first on A, then B, then C, as shown below.\\
	$$\includegraphics[width=90mm]{Q1PB.jpg}$$
	...
	\item When B = 0 we split on C because it has higher information gain than D. Since there are no cases where A = 0, B = 0 and C = 1, we return the majority Label = 1. Since there are still too many possible labels for C = 0, we split on D.
		\begin{itemize}
		\item Since there are no examples where A = 0, B = 0, C = 0, D = 0, we return the majority Label = 1.
		\item Since there are equal number of examples for both possible labels for D = 0, we broke this tie be returning the majority label, in this case, Label = 1. 
		\end{itemize}
	\end{itemize}
	...
	\item Analyze the effectiveness of boosting:
		\begin{enumerate}
			...
			\item Effect of number of boosting rounds on cross-validated performance of decision trees
			 $$\includegraphics[width=150mm]{nguyen-ming-boosting.pdf}$$
