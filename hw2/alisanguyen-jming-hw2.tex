...
\item
$$\includegraphics[width=130mm]{nguyen-ming-36b.pdf}$$ 
\begin{enumerate}
...
\\It is important that we use a validation set rather than the actual test set to tune the number of epochs because the test set should remain untouched until we are ready to evaluate performance to present more accurate results and so we can minimize the error accurately.
\end{enumerate}
...
\item We determine when to stop training by checking the improvement between the current epoch validation performance and the validation performance for the previous epoch. If the difference is less than a given $\epsilon$, which in this case was 0.0001, at least five times in a row, then stop the training.
\item $$\includegraphics[width=130mm]{nguyen-ming-37c.pdf}$$
...
\item Our new network's test performances include:
	...
	\item For a neural network with 3 hidden layers of 10 nodes each using learning rate of 0.3 and the convergence stopping algorithm: 
		...
		* * * * * * * * *
		...
		\end{verbatim}
	\end{itemize}
\item It was surprising, at first that the performances were so drastically low to begin with and they stopped pretty quickly after the minimum five rounds, meaning they did not improve significantly. However, thinking more about the data, it seemed to make more sense that adding more hidden layers would complicate the problem unecessarily and would not actually improve performance. Even though the absolute performance of our custom network is significantly worse than that of the simple network and hidden network, it was enlightening to realize these results and attempt to find explanations for them based on what we learned from class.
\end{enumerate}
