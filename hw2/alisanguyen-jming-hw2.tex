...
=======
\item We decided that we wanted to use a learning rate of 0.3 for 15 fully connected units in the hidden layer and 0.3 for 30 fully connected units in the hidden layer based on experimental trials with different learning rates.
\item We determine when to stop training by checking the difference between the validation performance and the previous validation performance. If the difference is less than a given $\epsilon$, which in this case was 0.001, at least five times in a row, then stop the training.
\item GRAPH HERE
\item We used FILLIN epochs for 15 hidden units and 28 epochs for 30 hidden units.
...
>>>>>>> 97068fb1c10e71c5d709776ceeee444f1bf53e9f
\end{enumerate}
8. CustomNetwork
\begin{enumerate}
...
\item Changing error function
	...
	\item Our weight update rule for the error function $C$ derived from gradient descent is:
		\[w_k^{(r+1)}\leftarrow w_k^{(r)} + 2 \alpha \lambda w_k + x_k\]
	\end{enumerate}
